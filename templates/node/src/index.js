...
async function initializeLogger() {
  try {
    // Logger is in the .logs directory at the service root (same level as src/)
    const loggerPath = path.join(__dirname, '..', '.logs', 'logger.js');
    if (fs.existsSync(loggerPath)) {
      // Use dynamic import with absolute file:// URL for ESM compatibility
      const { Logger } = await import(`file://${path.resolve(loggerPath)}`);
